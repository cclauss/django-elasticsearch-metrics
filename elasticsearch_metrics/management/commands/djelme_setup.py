...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--imp",
            type=str,
            help=f"name of a djelme imp, as named in settings.{IMPS_SETTING}",
        )
    ...
    def handle(self, *args, **options):
        ...
        if _imp_name:
            self.stdout.write(f"Using djelme imp {_imp_name!r}")
        for app_label in app_labels:
            ...
            _each_recordtype = (
                registry.each_recordtype(app_label=app_label, imp_name=_imp_name)
                if _imp_name
