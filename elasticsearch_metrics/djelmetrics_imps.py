...
def get_djelmetrics_imp(imp_name: str) -> ProtoDjelmetricsImp:
    ...
    try:
        ...
        raise ValueError(f'could not import {_imp_path!r} for `settings.DJELMETRICS_IMPS[{imp_name!r}]`')
    assert isinstance(_imp_module, ProtoDjelmetricsImpModule)
    _imp_config = {_k: _v for _k, _v in _raw_imp_config.items() if _k != 'DJELMETRICS_IMP'}
